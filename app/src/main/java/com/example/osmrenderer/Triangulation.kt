...
object Triangulation {
     ...
     */
    @JvmOverloads
    fun earcut(data: FloatArray, holeIndices: IntArray? = null, dim: Int = 2): ShortArray {
        val hasHoles = holeIndices != null && holeIndices.size > 0
        val outerLen = if (hasHoles) holeIndices!![0] * dim else data.size
        var outerNode = linkedList(data, 0, outerLen, dim, true)
        val triangles = mutableListOf<Short>()
        if (outerNode == null || outerNode.next === outerNode.prev) return triangles.toShortArray()
        var minX = 0.0F
        var minY = 0.0F
        var maxX = 0.0F
        var maxY = 0.0F
        ...
        if (data.size > 80 * dim) {
            ...
            invSize = if (invSize != 0.0F) 1.0F / invSize else 0.0F
        }
        earcutLinked(outerNode, triangles, dim, minX, minY, invSize, Int.MIN_VALUE)
        return triangles.toShortArray()
    }

    private fun earcutLinked(
        ear: Node,
        triangles: MutableList<Short>,
        dim: Int,
        minX: Float,
        minY: Float,
        invSize: Float,
        ...
        if (pass == Int.MIN_VALUE && invSize != Float.MIN_VALUE) indexCurve(
            ear!!,
            minX,
            minY,
            invSize
        ...
        while (ear!!.prev !== ear!!.next) {
            val prev = ear!!.prev
            val next = ear.next
            if (if (invSize != Float.MIN_VALUE) isEarHashed(ear, minX, minY, invSize) else isEar(
                    ear
                )
            ) {
                // cut off the triangle
                triangles.add((prev!!.i / dim).toShort())
                triangles.add((ear.i / dim).toShort())
                triangles.add((next!!.i / dim).toShort())
                removeNode(ear)
            ...
            b
                ...
                (locallyInside(a, b) && locallyInside(b, a) && middleInside(
                    ...
                    b
                        ...
                        (area(a.prev, a, b.prev) != 0.0F || area(
                            a,
                            b.prev,
                            b
                        ) != 0.0F) ||  // does not create opposite-facing sectors
    ...
    private fun intersects(p1: Node?, q1: Node?, p2: Node?, q2: Node?): Boolean {
        ...
        if (o1 == 0.0F && onSegment(
                p1,
                p2,
                q1
            )
        ) return true // p1, q1 and p2 are collinear and p2 lies on p1q1
        if (o2 == 0.0F && onSegment(
                p1,
                q2,
                q1
            )
        ) return true // p1, q1 and q2 are collinear and q2 lies on p1q1
        if (o3 == 0.0F && onSegment(
                p2,
                p1,
                q2
            )
        ) return true // p2, q2 and p1 are collinear and p1 lies on p2q2
        return if (o4 == 0.0F && onSegment(
                p2,
                q1,
                q2
            )
        ) true else false // p2, q2 and q1 are collinear and q1 lies on p2q2
    }
    ...
    private fun sign(num: Float): Float {
        return if (num > 0) 1.0F else if (num < 0) -1.0F else 0.0F
    }

    private fun cureLocalIntersections(start: Node?, triangles: MutableList<Short>, dim: Int): Node? {
        var start = start
        var p = start
        do {
            val a = p!!.prev
            ...
            if (!equals(a, b) && intersects(a, p, p.next, b) && locallyInside(
                ...
                triangles.add((a!!.i / dim).toShort())
                triangles.add((p.i / dim).toShort())
                triangles.add((b!!.i / dim).toShort())
    ...
    private fun isEarHashed(ear: Node?, minX: Float, minY: Float, invSize: Float): Boolean {
        ...
        val minTX =
            if (a!!.x < ear.x) if (a.x < c!!.x) a.x else c.x else if (ear.x < c!!.x) ear.x else c.x
        val minTY =
            if (a.y < ear.y) if (a.y < c.y) a.y else c.y else if (ear.y < c.y) ear.y else c.y
        val maxTX =
            if (a.x > ear.x) if (a.x > c.x) a.x else c.x else if (ear.x > c.x) ear.x else c.x
        val maxTY =
            if (a.y > ear.y) if (a.y > c.y) a.y else c.y else if (ear.y > c.y) ear.y else c.y
    ...
    private fun eliminateHoles(
        ...
        queue.sortWith(object : Comparator<Node?> {
            override fun compare(o1: Node?, o2: Node?): Int {
                if (o1!!.x - o2!!.x > 0) return 1 else if (o1.x - o2.x < 0) return -2
                return 0
            }
        })
        for (node in queue) {
    ...
    private fun locallyInside(a: Node?, b: Node?): Boolean {
        return if (area(a!!.prev, a, a.next) < 0) area(a, b, a.next) >= 0 && area(
            a,
            a.prev,
            b
        ) >= 0 else area(a, b, a.prev) < 0 || area(a, a.next, b) < 0
    }
    ...
    private class Node internal constructor(var i: Int, var x: Float, var y: Float) {
        ...
        override fun toString(): String {
            ...
            sb.append("{i: ").append(i).append(", x: ").append(x).append(", y: ").append(y)
                .append(", prev: ").append(prev).append(", next: ").append(next)
            return sb.toString()
        }
